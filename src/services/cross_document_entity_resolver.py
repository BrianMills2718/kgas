...
class CrossDocumentEntityResolver:
    ...
    async def _llm_cluster_entities(
        ...
        try:
            ...
            self.logger.error(f"LLM clustering failed: {e}")
            # FAIL-FAST: Don't hide LLM failures behind fallbacks
            raise RuntimeError(
                f"CrossDocumentEntityResolver requires LLM service for clustering. "
                f"LLM clustering failed: {e}. "
                f"Fix the LLM service or use a different entity resolution approach."
            )
...
"""
    ...
    # REMOVED: _fallback_heuristic_clustering 
    # Reason: Violates fail-fast principle by hiding LLM service failures
    # If LLM clustering fails, the service should fail clearly rather than
    # silently falling back to inferior heuristic clustering
