    ...
    def execute(self, request: ToolRequest) -> ToolResult:
        ...
        try:
            ...
            if self.service_manager and hasattr(self.service_manager, 'provenance_service'):
                operation_id = self.service_manager.provenance_service.start_operation(
                    tool_id=self.tool_id,
                    operation_type="tool_execution",
                    inputs=[str(type(request.input_data).__name__)],
                    parameters={
                        "workflow_id": request.workflow_id,
                        "execution_time": execution_time
                    }
                )
                provenance_record = self.service_manager.provenance_service.complete_operation(
                    operation_id=operation_id,
                    outputs=[str(type(result).__name__)],
                    success=True
                )
