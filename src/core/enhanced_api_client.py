...
class EnhancedAPIClient:
    ...
    def _load_model_configs(self) -> Dict[str, ModelConfig]:
        ...
        default_models = {
            ...
            'gemini_flash': 'gemini/gemini-2.5-flash-lite,true,8192'  # Enable structured output
        }
    ...
    def _load_fallback_config(self) -> Dict[str, Any]:
        ...
        return {
            'primary_model': os.getenv('PRIMARY_MODEL', 'gemini_flash'),  # Use Gemini as primary
            'fallback_models': [
                os.getenv('FALLBACK_MODEL_1', 'gpt_4o_mini'),
    ...
    def generate_text(self, prompt: str, model: str = None, max_tokens: int = None, 
                     temperature: float = None, **kwargs) -> Dict[str, Any]:
        """Generate text using LLM (compatibility method for WorkflowAgent)
        
        This method provides backward compatibility with components expecting
        a generate_text interface. It wraps the make_request method.
        
        Args:
            prompt: Text prompt for generation
            model: Model name to use (defaults to first available)
            max_tokens: Maximum tokens to generate
        ...
        try:
            ...
            if model is None:
                # Try to use Gemini 2.5 Flash as default, fallback to first available
                if 'gemini-2.5-flash' in self.models:
                    model = 'gemini-2.5-flash'
                elif 'gemini/gemini-2.5-flash' in self.models:
                    model = 'gemini/gemini-2.5-flash'
                else:
                    # Use first available model
                    model = list(self.models.keys())[0] if self.models else 'gpt-3.5-turbo'
            
            # Make the request using existing infrastructure
