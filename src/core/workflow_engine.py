...
class WorkflowEngine:
    ...
    def _execute_step(self, step: WorkflowStep, step_execution: StepExecution, execution: WorkflowExecution):
        ...
        try:
            if step.step_type == WorkflowStepType.TOOL_EXECUTION:
                ...
                # Map outputs to workflow data - handle different result types
                if step.output_mapping:
                    for output_key, workflow_key in step.output_mapping.items():
                        value = None
                        
                        # Try to get value from result.data
                        if hasattr(result, 'data') and isinstance(result.data, dict):
                            value = result.data.get(output_key)
                        # Fallback to treating result as dict
                        elif isinstance(result, dict) and 'data' in result and isinstance(result['data'], dict):
                            value = result['data'].get(output_key)
                        # Direct dict access
    ...
    def _execute_tool_step(self, step: WorkflowStep, execution: WorkflowExecution) -> ToolResult:
        ...
        # Log execution - handle different result types safely
        status = getattr(result, 'status', result.get('status', 'unknown') if isinstance(result, dict) else 'unknown')
        ...
        execution.execution_log.append(
            f"Tool {step.tool_id} executed: status={status}, confidence={confidence}"
        )
