...
class SequentialEngine:
        ...
        try:
            for i, tool in enumerate(tools):
                ...
                try:
                    ...
                    self.logger.info(f"Executing tool {i+1}/{len(tools)}: {tool.tool_name} ({tool.tool_id})")
                    ...
                    execution_result = {
                        ...
                        "status": "success",
                        "result_summary": self._summarize_result(result.data),
                    ...
                    tool_execution_time = time.time() - tool_start_time
                    error_msg = f"Tool {tool.tool_name} ({tool.tool_id}) failed: {str(e)}"
                    ...
                    if self._should_stop_on_error(tool, e):
                        ...
                        self.logger.warning(f"Continuing execution despite error in {tool.tool_name}")
                        continue
            
            total_execution_time = time.time() - start_time
            self.execution_stats["total_time"] = total_execution_time
            
            # Get final data from last result if available
            final_data = {}
            if execution_results and execution_results[-1]["status"] == "success":
                # Last tool executed successfully, use its result data
                final_data = current_request.input_data if hasattr(current_request, 'input_data') else {}
            ...
            return {
                ...
                "error": str(e),
                "execution_results": execution_results,
                "partial_data": current_request.input_data if hasattr(current_request, 'input_data') else {},
                "execution_stats": self.execution_stats.copy(),
                "total_execution_time": total_execution_time
            }
    ...
    def _should_stop_on_error(self, tool: KGASTool, error: Exception) -> bool:
