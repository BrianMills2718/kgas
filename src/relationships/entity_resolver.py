...
class EntityResolver:
    ...
    def _extract_entity_references(self, document: Dict[str, Any]) -> List[EntityReference]:
        ...
        # Extract authors from metadata first
        authors = metadata.get("authors", [])
        for author in authors:
            if author and isinstance(author, str):
                # Add author as an entity reference
                entity_ref = EntityReference(
                    entity_id=f"{doc_path}_author_{len(entity_refs)}",
                    entity_name=author.replace("Dr. ", "").replace("Prof. ", ""),  # Store clean name
                    document_path=doc_path,
                    mention_context=content if content else f"Author of document: {doc_path}",  # Use full content as context
                    confidence_score=0.95,  # High confidence for authors
                    entity_type="person",
                    position_in_doc=0,
                    aliases=[author]  # Store full author name with title as alias
                )
                entity_refs.append(entity_ref)
        
        # First extract full names and titles from content
        ...
        # Also include author names for partial matching
        author_names = {}
        for author in authors:
            if author and isinstance(author, str):
                clean_name = author.replace("Dr. ", "").replace("Prof. ", "")
                if clean_name not in full_names:
                    full_names[clean_name] = []
                    # Add the author name to the full_names dict for partial matching
                    if ' ' in clean_name:
                        parts = clean_name.split()
                        last_name = parts[-1]
                        first_name = parts[0]
                        full_names[clean_name].append(last_name)
                        full_names[clean_name].append(first_name)
        
        # Look for partial name references (e.g., "Dr. Chen" after we know about "Sarah Chen" or "James Chen")
        ...
        # Authors already extracted at the beginning of the method
