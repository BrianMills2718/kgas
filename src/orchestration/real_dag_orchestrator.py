...
class RealDAGOrchestrator:
    ...
    def _initialize_tools(self) -> Dict[str, Any]:
        ...
        tools['T23C_ONTOLOGY_EXTRACTOR'] = OntologyAwareExtractor(self.service_manager)
    ...
    async def execute_node(self, node_id: str, input_data: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            if node.tool_name == "T01_PDF_LOADER":
            ...
            elif node.tool_name == "T23C_ONTOLOGY_EXTRACTOR":
                ...
                # T23c expects text and ontology
                tool_request = ToolRequest(
                    tool_id="T23C_ONTOLOGY_EXTRACTOR",
                    operation="extract",
                    input_data={
                        "text": " ".join([c.get('text', '') for c in chunks]) if chunks else "",
                        "source_ref": f"chunk_{node_id}",
                        "ontology": None  # Will use default ontology
                    },
                    parameters={}
                )
            else:
...
async def demo_real_dag_execution():
    ...
    # Extract entities and relationships together with T23c
    orchestrator.add_node("extract_all", "T23C_ONTOLOGY_EXTRACTOR", inputs=["chunk_text"])
