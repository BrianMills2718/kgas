...
class VectorConverter(BaseConverter):
        ...
        try:
            ...
            if self.embedding_service:
                try:
                    ...
                    error_msg = "Embedding service circuit breaker open. Fix: Check embedding service health and configuration."
                    logger.critical(error_msg)
                    raise Exception(error_msg)
                except Exception as e:
                    error_msg = f"Embedding service failed after retries: {e}. Fix: Check embedding service configuration and connectivity."
                    logger.critical(error_msg)
                    raise Exception(error_msg)
            else:
                # FAIL-FAST: No embedding service available
                error_msg = "Embedding service required for graph to vector conversion. Fix: Configure embedding service (OPENAI_API_KEY, GOOGLE_API_KEY, or ANTHROPIC_API_KEY) and verify service health."
                logger.critical(error_msg)
    ...
    async def _create_row_embeddings(self, df: pd.DataFrame) -> np.ndarray:
        ...
        try:
            ...
            if self.embedding_service:
                ...
                # FAIL-FAST: No embedding service available
                error_msg = "Embedding service required for table to vector conversion. Fix: Configure embedding service (OPENAI_API_KEY, GOOGLE_API_KEY, or ANTHROPIC_API_KEY) and verify service health."
                logger.critical(error_msg)
                raise ServiceUnavailableError(error_msg)
    ...
    # REMOVED: _create_hash_embeddings() method - FAIL-FAST ARCHITECTURE
    # Hash-based embeddings provide meaningless similarity scores that mislead users
    # System now fails clearly when embedding service is unavailable
...
class CrossModalConverter(CoreService):
    ...
    async def batch_convert_parallel(
        self,
        data_items: List[Any],
        source_format: DataFormat,
        ...
        **kwargs
        ...
        NOTE: Renamed from batch_convert_with_fallback to align with fail-fast architecture.
        This method uses parallel processing, not fallback logic.
        """
