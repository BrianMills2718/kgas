...
**Categories**: EXPRESSIONS, ALGORITHMS, PROCESSES, RULES
...
|----------|-------------|-------------------|
| EXPRESSIONS | `algorithms.mathematical` | Prospect Theory value function |
| ALGORITHMS | `algorithms.procedural` | PageRank calculation |
| PROCESSES | `algorithms.procedural.steps` | Decision-making workflow |
...
#### 3.1 EXPRESSIONS → Direct Mathematical Functions
```python
...
#### 3.2 ALGORITHMS → Iterative Computational Methods  
```python
...
#### 3.3 PROCESSES → State-Based Workflows
```python
# From V12: Decision procedure with state transitions
class RationalChoiceDecisionProcess:
    def execute(self, context):
        ...
        while state != "complete":
            if state == "identify":
                ...
                result = self.select_optimal(utilities)
                state = "complete"
        return result
```

#### 3.4 RULES → Logical Inference
```python
...
- **[V13 Meta-Schema](config/schemas/theory_meta_schema_v13.json)**: Complete schema specification
