...
- **`systems/`**: Detailed design of major system components
  - **`two-layer-theory-architecture.md`**: Theory extraction design specification
  - **`theory-extraction-integration.md`**: Integration architecture for experimental system
...
### 2. Two-Layer Theory Architecture
- **Layer 1**: Comprehensive theoretical structure extraction independent of analytical goals
- **Layer 2**: Question-driven analysis using extracted theoretical structures
- **Experimental Status**: Fully functional system in `/experiments/lit_review` with 100% validation success
- **Integration Required**: Connect experimental system to main KGAS architecture via wrapper pattern

### 3. Cross-Modal Analysis Architecture
...
- **ADR-030**: Research System Simplification (Focus on research needs over enterprise patterns)
- **Future ADRs**: Cross-modal orchestration, theory integration, performance optimization

## Lessons Learned & Implementation Guidance

### **Implementation Philosophy**
These lessons from KGAS development guide future implementation decisions:

#### **1. Connect Existing Capabilities First**
- **Prioritize integration** of existing sophisticated components over building new ones
- **Register tools** in the tool registry before creating new tools
- **Link infrastructure** that already exists before developing alternatives
- **Example**: The analytics infrastructure (16,798 lines) exists but needs registration - connect it rather than building new analytics

#### **2. Match Patterns to System Purpose**
- **Choose patterns appropriate for research systems** rather than enterprise software
- **Implement direct service access** when dependency injection isn't needed
- **Use simple configuration** when multi-environment deployment isn't required
- **Example**: Standard ServiceManager with FAIL-FAST philosophy fits research needs better than complex dependency injection

#### **3. Validate Actual Requirements**
- **Confirm features solve real problems** before implementation
- **Test if simpler approaches suffice** before adding complexity
- **Verify multi-user/multi-environment needs** before building for them
- **Example**: Single research environment doesn't need dev/test/prod configuration complexity

#### **4. Maintain Clear Architecture Boundaries**
- **Keep target architecture aspirational** in architecture docs
- **Document current state** in roadmap/implementation docs
- **Track implementation progress** separately from design goals
- **Avoid mixing** implementation status with architectural vision

#### **5. Start with Minimal Viable Integration**
- **Begin with core service connections** before adding sophisticated features
- **Implement basic tool registration** before advanced discovery
- **Create simple access patterns** before complex abstractions
- **Example**: Register 5 critical cross-modal tools before attempting all 122+

#### **6. Recognize Over-Engineering Early**
- **Monitor for features without use cases** during development
- **Question enterprise patterns** in research systems
- **Identify unused sophisticated components** and evaluate their necessity
- **Example**: Enhanced ServiceManager built but never used indicates over-engineering

#### **7. Embrace Research System Constraints**
- **Design for single-node execution** rather than distributed systems
- **Optimize for flexibility** over performance in research contexts
- **Build for experimentation** rather than production deployment
- **Accept research-appropriate trade-offs** in design decisions

### **Integration Before Innovation**
The most important lesson: **Sophisticated capabilities already exist in KGAS** - they just need to be connected. Focus implementation efforts on:
1. Making existing tools discoverable
2. Connecting isolated infrastructure
3. Simplifying access patterns
4. Removing unnecessary abstractions

This approach yields immediate capability gains while reducing system complexity.
