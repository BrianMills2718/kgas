#!/usr/bin/env python3
"""
TEST MCP TOOLS BY TESTING THE UNDERLYING SERVICES DIRECTLY
Since MCP tools are wrapped, test the actual service functions they call
"""
...
def test_mcp_services():
    ...
    print("🔥 TESTING MCP TOOL UNDERLYING SERVICES")
    print("=" * 80)
    ...
    services = [
        {"name": "Identity Service", "test_func": test_identity_service},
        {"name": "Provenance Service", "test_func": test_provenance_service},
        {"name": "Quality Service", "test_func": test_quality_service},
        {"name": "Workflow State Service", "test_func": test_workflow_service},
        {"name": "Vertical Slice Workflow", "test_func": test_vertical_slice},
        {"name": "Phase 1 MCP Tools", "test_func": test_phase1_mcp_tools},
    ]
    ...
    # Save results
    with open("mcp_services_test_results.json", "w") as f:
        json.dump(results, f, indent=2)
    
    print(f"\n📄 Detailed results saved to: mcp_services_test_results.json")
    
    return results

def test_identity_service():
    """Test Identity Service functionality"""
    try:
...
def test_provenance_service():
    """Test Provenance Service functionality"""
    try:
        ...
        return f"Provenance Service working: {len(stats)} tools tracked"
...
def test_workflow_service():
    """Test Workflow State Service functionality"""
    try:
...
def test_phase1_mcp_tools():
    """Test Phase 1 MCP Tools creation"""
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            # Test tool creation
            tools = create_phase1_mcp_tools(workflow_storage_dir=temp_dir)
