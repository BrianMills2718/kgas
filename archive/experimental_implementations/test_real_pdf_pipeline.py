...
print("=== FULL PIPELINE TEST WITH REAL PDF ===\n")
...
print(f"  Document ID: {pdf_result['document_ref']}")
...
for i, chunk_ref in enumerate(chunk_result['chunk_refs'][:10]):  # Limit to first 10 chunks for cost
    print(f"  Processing chunk {i+1}/{min(10, len(chunk_result['chunk_refs']))}...")
    try:
...
embedder.generate_embeddings(object_refs=chunk_result['chunk_refs'][:10])
print(f"✓ Generated embeddings for chunks")

# Get unique entities for embedding
with db.neo4j.driver.session() as session:
    result = session.run("MATCH (e:Entity) RETURN 'neo4j://entity/' + e.id as ref LIMIT 50")
...
test_queries = [
    "What companies are mentioned in the paper?",
    "Who invested in OpenAI?",
    "What databases does GraphRAG use?",
