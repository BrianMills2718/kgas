#!/usr/bin/env python3
"""
TEST FILE FOR CAPABILITY 016
Capability: TextChunker._create_overlapping_chunks()
Description: Create chunks with specified overlap
File: src/tools/phase1/t15a_text_chunker.py
Class: TextChunker
Method: _create_overlapping_chunks
...
def test_capability_016():
    """Test Capability 016: TextChunker._create_overlapping_chunks()"""
    
    test_start_time = time.time()
    result = {
        "capability_id": 16,
        "capability_name": "TextChunker._create_overlapping_chunks()",
        "test_type": "private_method",
        "description": "Create chunks with specified overlap",
        "file_path": "src/tools/phase1/t15a_text_chunker.py",
        "class_name": "TextChunker",
        "method_name": "_create_overlapping_chunks",
    ...
    try:
        print(f"🧪 TESTING CAPABILITY 016: TextChunker._create_overlapping_chunks()")
        print(f"📁 File: src/tools/phase1/t15a_text_chunker.py")
        print(f"🏗️  Class: TextChunker")
        print(f"⚙️  Method: _create_overlapping_chunks")
        print(f"📝 Description: Create chunks with specified overlap")
        print("-" * 80)
...
def main():
    ...
    result = test_capability_016()
    
    # Save result to log file
    log_file = f"capability_016_test_log.txt"
    
    with open(log_file, 'w') as f:
        f.write(f"CAPABILITY 016 TEST LOG\n")
        f.write("=" * 50 + "\n")
        f.write(f"Capability: TextChunker._create_overlapping_chunks()\n")
        f.write(f"Description: Create chunks with specified overlap\n")
        f.write(f"File: src/tools/phase1/t15a_text_chunker.py\n")
        f.write(f"Class: TextChunker\n")
        f.write(f"Method: _create_overlapping_chunks\n")
