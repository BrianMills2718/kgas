...
def export_to_obsidian(graph_data, vault_path):
    ...
    with open(os.path.join(export_dir, "graph_summary.md"), 'w', encoding='utf-8') as f:
...
if st.button("Update Graph") and user_instruction.strip():
    st.session_state.current_graph = update_knowledge_graph(user_instruction, st.session_state.current_graph)
...
if obsidian_vault_path:
    ...
    if st.sidebar.button("Export to Obsidian"):
        try:
            ...
            st.success("Successfully exported graph to Obsidian vault!")
        except Exception as e:
