#!/usr/bin/env python3
"""
Simple Gemini-Focused MCP Tool Selection Testing

Uses direct LiteLLM calls with structured output for reliable JSON parsing.
...
class SimpleGeminiTester:
    ...
    def test_gemini_structured(self, 
                             tools: List[Dict[str, Any]], 
                             scenario: TestScenario,
                             methodology: str = "direct_exposure") -> GeminiTestResult:
        """Test Gemini with structured output using direct LiteLLM"""
        ...
        try:
            ...
            # Extract results (handle flexible JSON structure)
            selected_tools = []
            selection_reasoning = []
            confidence_score = 0.8  # Default
            
            if "selected_tools" in response_data:
                for tool_selection in response_data["selected_tools"]:
                    if isinstance(tool_selection, dict):
                        selected_tools.append(tool_selection.get("tool", "unknown"))
                        selection_reasoning.append(tool_selection.get("reasoning", ""))
                    else:
                        selected_tools.append(str(tool_selection))
                        selection_reasoning.append("")
            
            if "selection_confidence" in response_data:
                confidence_score = response_data["selection_confidence"]
...
{{
    "selected_tools": [
        {{
            ...
            "reasoning": "why this tool is essential",
            "confidence": 0.9
        }}
    ],
    "overall_strategy": "your approach to this task",
...
Focus on tools that actually exist and can work together to complete the task.
    ...
    def run_cognitive_load_analysis(self) -> List[GeminiTestResult]:
        ...
        for tool_count in self.tool_counts:
            ...
            result = self.test_gemini_structured(
    ...
    def run_methodology_comparison(self, viable_tool_counts: List[int]) -> List[GeminiTestResult]:
        ...
        for methodology in self.methodologies:
            ...
            for tool_count in viable_tool_counts[:3]:  # Limit to avoid too many API calls
                for scenario in self.test_scenarios[:2]:  # Limit scenarios
    ...
    def analyze_results_for_kgas(self, 
        ...
        if successful_cognitive:
            max_viable_tools = max(r.tool_count for r in successful_cognitive 
                                 if r.accuracy_metrics['f1'] > 0.2)  # Reasonable accuracy threshold
        ...
        # Best methodology
        best_methodology = "direct_exposure"  # Default fallback
        if methodology_performance:
            best_methodology = max(methodology_performance.keys(), 
                                 key=lambda m: methodology_performance[m]["avg_f1_score"])
        ...
        kgas_recommendations = {
            "cognitive_load_analysis": {
                ...
                "successful_tests": len(successful_cognitive),
                "total_tests": len(cognitive_load_results)
            },
    ...
    def run_comprehensive_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Phase 2: Methodology Comparison (limited to avoid too many API calls)
            methodology_results = self.run_methodology_comparison(viable_counts)
            
            # Phase 3: KGAS Analysis
            kgas_analysis = self.analyze_results_for_kgas(cognitive_load_results, methodology_results)
            ...
            final_results = {
                "test_summary": {
                    ...
                    "success_rate": len([r for r in cognitive_load_results + methodology_results if r.selection_success]) / len(cognitive_load_results + methodology_results) if (cognitive_load_results + methodology_results) else 0,
                    "completion_timestamp": time.time()
                },
                "cognitive_load_analysis": [asdict(r) for r in cognitive_load_results],
                "methodology_comparison": [asdict(r) for r in methodology_results],
    ...
    def save_results(self, results: Dict[str, Any], filename: Optional[str] = None) -> str:
        ...
        if not filename:
            ...
            filename = f"simple_gemini_analysis_{timestamp}.json"
...
def main():
    ...
    test_runner = SimpleGeminiTester()
    
    try:
        results = test_runner.run_comprehensive_analysis()
